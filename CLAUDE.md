...
**MANDATORY: Never bump version number in package.json without explicit permission!**

- ❌ Do NOT change version after completing features
- ❌ Do NOT bump version when code is "ready"
- ✅ ONLY bump version when user says "ready to push to GitHub"

**Why:**
- Version bumps happen as part of the release process
- User batches multiple features before creating a new release
- Version number signals "this is going to GitHub now"

**Process:**
1. Complete features/fixes → Keep version same
2. User says "ready to push" → Ask if version should be bumped
3. User confirms → Bump version, commit, push

---

### Git/Documentation Policy

**ONLY commit these two documentation files:**
- ✅ `CHANGELOG.md` - Version history
- ✅ `README.md` - User-facing documentation

**FORBIDDEN - Keep LOCAL ONLY:**
- ❌ `CLAUDE.md` - Project instructions (this file) - **DO NOT PUSH TO GITHUB**
...
**Why:** User wants minimal repo - only essential user-facing files in git. CLAUDE.md is for development/Claude only, not for the public repo.

**How to document:**
- Add notes to `CLAUDE.md` for developers/Claude (local only)
- Add to `CHANGELOG.md` for version history (committed)
- Add to `README.md` for end users (committed)
- Everything else stays local (don't commit)

---

## 🚨 DEBUGGING METHODOLOGY

**MANDATORY: When bugs occur, follow this protocol BEFORE making changes:**

### Step 1: Read the Logs First
- **ALWAYS** read `devLog.log` and `AppLog.txt` before diagnosing
- Look for: error messages, warnings, stack traces, infinite loops
- Note: what happened, when it happened, what was the trigger

### Step 2: Identify Root Cause
- **DON'T** guess or make immediate changes
- **DO** analyze the error pattern:
  - Is it a React infinite loop? → Check useEffect dependencies, Zustand selectors
  - Is it a state update issue? → Check store subscriptions, selector stability
  - Is it a timing issue? → Check async operations, race conditions
  - Is it a TypeScript error? → Check type definitions, generic constraints

### Step 3: Think Critically About Solutions
- **Ask yourself:**
  - What is the ACTUAL problem? (not just the symptom)
  - Why is this happening? (mechanism, not just observation)
  - What are 2-3 possible solutions?
  - Which solution addresses the root cause?
- **Discuss with user BEFORE coding** if uncertain about root cause
...
- **Format:**
  ...
  Proposed Fix: [Specific approach]
...
- Don't bundle multiple fixes unless they're related

### Common Patterns to Watch For

**React Infinite Loops:**
- Zustand selectors that change identity every render
- useEffect with missing/incorrect dependencies
- setState calls inside render

**Solution patterns:**
- Use single selector for entire state object, then destructure
- Memoize callbacks with useCallback
- Move calculations outside render

**TypeScript Type Errors:**
- Generic type inference failures
- `any` types causing cascading issues
- Missing type imports

**Solution patterns:**
- Explicitly type function returns
- Import types from source
- Use `ReturnType<typeof fn>` for inference

**State Management Issues:**
- Stale closures in callbacks
- Race conditions in async updates
- Store subscription not triggering re-renders

**Solution patterns:**
- Use store selectors instead of destructuring
- Add proper dependency arrays
- Ensure selector functions are stable
...
## Current Features (v0.8.0 - IN DEVELOPMENT June 2, 2026)

### 🔄 Major Update: Multi-Tab Session System

**Status:** Core implementation complete, testing in progress

**Completed:**
1. ✅ **Multi-tab infrastructure** - Up to 6 concurrent project tabs
2. ✅ **Per-tab state isolation** - Each tab has separate chat history, file tree, settings
3. ✅ **Tab management UI** - TabBar with create/close/switch, context menu, keyboard shortcuts
4. ✅ **Permission system fixes** - Per-tab permissions with backend synchronization
5. ✅ **Sessions moved to sidebar** - Per-tab session browser (project-specific)
6. ✅ **Zustand reactivity fixes** - Single-selector pattern prevents infinite loops
7. ✅ **Project selector improvements** - Smart welcome screen, automatic show/hide based on tab count

**Known Issues:**
1. ⚠️ **Tool cards showing "{}"** - Tool input data not being emitted from Rust backend (StreamEvent missing input field)
2. ⚠️ **No thinking indicators** - Need to verify thinking status is wired to tab-specific state

**Next Steps:**
1. Fix tool card input display (add `tool_input` to Rust StreamEvent)
2. Verify/fix thinking indicators for tab-aware rendering
3. Full end-to-end testing of multi-tab workflows
4. Update version to 0.8.0 and prepare release

### ✅ Core Features (All Working)
1. **Chat interface** - Claude CLI streaming with full event handling
2. **Permission system** - Per-tab permissions with 6 modes, backend sync on tab create/switch
3. **Thinking indicators** - Status updates with timer display (may need tab-aware verification)
